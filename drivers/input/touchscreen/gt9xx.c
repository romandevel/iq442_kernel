...
#if GTP_DEBUG_ON
    static const int  key_codes[] = {KEY_HOME, KEY_BACK, KEY_MENU};
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	cfg_ver = send_cfg_buf[sensor_id][0];
#endif
        if (opr_buf[0] < 90)    
        {
...
static  s32 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    memcpy(ic_msg.ic_fw_msg.hw_info, &buf[GTP_ADDR_LENGTH], 4);
    for(i=0; i<4; i++)
    {
        ic_msg.ic_fw_msg.hw_info[i] = buf[GTP_ADDR_LENGTH + 3 - i];
    } 
    GTP_DEBUG("IC Hardware info:%02x%02x%02x%02x", ic_msg.ic_fw_msg.hw_info[0], ic_msg.ic_fw_msg.hw_info[1],
                                                   ic_msg.ic_fw_msg.hw_info[2], ic_msg.ic_fw_msg.hw_info[3]);
    ...
    for(retry=0; retry<2; retry++)
    {
        ...
        ic_msg.force_update = buf[GTP_ADDR_LENGTH];
        if((0xBE != ic_msg.force_update)&&(!retry))
        {
    ...
    GTP_DEBUG("IC force update flag:0x%x", ic_msg.force_update);
    ...
    memset(ic_msg.ic_fw_msg.pid, 0, sizeof(ic_msg.ic_fw_msg.pid));
    memcpy(ic_msg.ic_fw_msg.pid, &buf[GTP_ADDR_LENGTH], 4);
    GTP_DEBUG("IC Product id:%s", ic_msg.ic_fw_msg.pid);
    ...
    /*|-----FLASH-----RAM-----|
      ...
      |------9110-----9110----|
      |------9110P----9111----|*/
    if(ic_msg.ic_fw_msg.pid[0] != 0)
    {
        if(!memcmp(ic_msg.ic_fw_msg.pid, "9111", 4))
        {
            GTP_DEBUG("IC Mapping Product id:%s", ic_msg.ic_fw_msg.pid);
            memcpy(ic_msg.ic_fw_msg.pid, "9110P", 5);
        }
    }
    
    ic_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH+4] + (buf[GTP_ADDR_LENGTH+5]<<8);
    GTP_DEBUG("IC version id:%04x", ic_msg.ic_fw_msg.vid);
...
gup_get_ic_fw_msg(client);

s +=sprintf(s,"IC Hardware info:0x%02x%02x%02x%02x\n", ic_msg.ic_fw_msg.hw_info[0], ic_msg.ic_fw_msg.hw_info[1],
                                                   ic_msg.ic_fw_msg.hw_info[2], ic_msg.ic_fw_msg.hw_info[3]);

s += sprintf(s,"IC force update flag:0x%x\n", ic_msg.force_update);

s += sprintf(s,"IC Product id:%s\n", ic_msg.ic_fw_msg.pid);
    
s += sprintf(s,"IC version id:0x%04x\n", ic_msg.ic_fw_msg.vid);
s += sprintf(s,"sensor id:0x%04x\n", cfg_sensor_id);
s += sprintf(s,"cfg file version:0x%04x\n", cfg_ver);
    
return (s - buf);
}

/*upgrade from *.i*/
static ssize_t gtp_information_store(struct device *dev,
					struct device_attribute *attr,
...
static DEVICE_ATTR(gtpinfo, S_IRUGO | S_IWUSR, gtp_information_show,
			gtp_information_store);

static struct attribute *goodix_attributes[] = {
	&dev_attr_gtpinfo.attr,
	NULL
};
...
int gtp_create_sysfs(struct i2c_client *client)
{
	int err;
	...
	if (0 != err) {
		...
		pr_info("GUP:%s() - sysfs_create_group() succeeded.\n",
				__func__);
	}
	return err;
}

void gtp_release_sysfs(struct i2c_client *client)
{
	sysfs_remove_group(&client->dev.kobj, &goodix_attribute_group);
}
#endif
